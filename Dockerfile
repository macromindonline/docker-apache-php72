...
RUN export DEBIAN_FRONTEND=noninteractive && apt-get update --fix-missing && apt-get -y install git curl apache2 php php7.2-mysql php7.2-curl php7.2-intl php7.2-json php7.2-imap php7.2-zip php7.2-gd php7.2-xml php7.2-mbstring libapache2-mod-php7.2 php7.2-sqlite3 php7.2-intl unzip && ln -fs /usr/share/zoneinfo/America/Sao_Paulo /etc/localtime && dpkg-reconfigure --frontend noninteractive tzdata && apt-get clean && rm -rf /var/lib/apt/lists/*
